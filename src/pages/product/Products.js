...
import { PencilAltIcon, TrashIcon, DotsVerticalIcon, PhotographIcon, SearchIcon } from "@heroicons/react/solid";
...
export default function Products() {
  ...
  /* const handleChecked = (e) => {
    ...
    if (e.target.checked) {
      const prfs = products.filter(
        (prf) => prf.category === e.target.value
      );
      dispatch(doGetProductRequest(prfs));
    } else {
      dispatch(doGetProductRequest(products));
    }
  }; */
  ...
  async function fetchData() {
    ...
    const payload = {
      ...
      category: categy.name[0] ,//prdct.category, //"Obat", //categy[0], //"Semua",  //
      is_active: "All",
  ...
  return (
    <>
      <div className="flex w-full mb-5">
        <label className="text-indigo text-xl text-purple-900s ml-5 mr-5 mt-2">
          <b className="shadow">DAFTAR PRODUK</b>
        </label>
      ...
      <div className="flex w-full mb-5">
        ...
        <div /* className="filter-container" */>
          <div className="mb-4">
            ...
            <select
              class="p-2 ml-3 mr-5 text-purple bg-gray-300 focus:ring-4 focus:ring-purple-300 font-medium text-sm px-5 py-2.5 text-justify"
              onChange="" /* {handleCategoryChange()} */
            >
              {categy.map((ctg) => (
                <option value="" /* {row.cells[3].value}*/> {ctg.name}</option>
              ))}
            </select>
          </div>
        </div>
                  ...
                  <div className="flex flex-row min-h-screen justify-center items-center">
                    ...
                    >
                      ...
                      <span className="relative inline-flex items-center px-2 py-2 bg-white text-sm font-medium text-gray-700">
                        Page <strong>{pageIndex + 1} of ?</strong>{" "}
                      </span>
                      <button
                        onClick={() => nextPage()}
                    ...
                    >
                      ...
                      {/* <span className="relative inline-flex items-center px-2 py-2 bg-white text-sm font-medium text-gray-700">
                                                <select value={pageSize} onChange={e => setPageSize(Number(e.target.value))}>
                                                    {  [5, 10, 100].map(pageSize => (      <option key={pageSize} value={pageSize}>          Show {pageSize}      </option>  ))
                                                    }
                                                </select>
                                            </span> */}
                      <button
